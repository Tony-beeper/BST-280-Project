...
Pancreatic cancer is one of the deadliest malignancies worldwide. It has a five-year survival rate of less than 10% due to its aggressive nature(Chen, 2023), limited treatment options and late diagnosis. Among pancreatic cancers, pancreatic adenocarcinoma (PAAD) accounts for the most of cases. It has challenges to early detection and therapeutic intervention. Understanding the molecular mechanisms driving PAAD is important for developing diagnostic markers and therapeutic targets to improve patient care(Wang, 2021).

This study aims to identify genes that are differentially expressed between normal pancreatic tissue and pancreatic adenocarcinoma tissue, using genetic data from The Cancer Genome Atlas (TCGA) and the Genotype-Tissue Expression (GTEx) project. By integrating advanced bioinformatics approaches, we seek to uncover main biological processes, pathways, and molecular functions associated with PAAD development and progression.

Our main research questions are as below:

What genes are differentially expressed between normal pancreatic tissue(GTEx) and pancreatic adenocarcinoma tissue(TCGA)?
...
With data preprocessing, differential gene expression analysis(DEGs), pathway enrichment analysis(GSEA), and network inference(PANDA), this project will provide insights into the biology of pancreatic adenocarcinoma. By stratifying data based on sex, diabetes status,and alcohol consumption, we further aim to investigate molecular features that may refine our understanding of PAAD heterogeneity and its clinical implications.

# Main Reseasrch Methods & Result: TCGA VS GTEx
...
# Secondary Research Methods & Result: Male vs Female
...
# Secondary Research Methods & Result: Diabetic VS Non-Diabetic
...
## Diabetic Analysis Conclusion

Our analysis comparing diabetic and non-diabetic cancer groups found no differentially expressed genes with a log-fold change of 1. However, network analysis with PANDA showed some interesting differences in gene networks. PINX1(You, 2024) was a positively targeted hub, and its over-regulation stood out. STC2(Lin, 2020) was found as a negatively targeted hub. The exact reasons behind these results are not clear, but they point to areas that could be worth exploring to better understand the effects of diabetes and cancer separately.

# Secondary Research Methods & Result: Alcohol Consumption
...
### KEGG(Lower targeting in alcoholics)
...
### KEGG(Higher targeting in alcoholics)
```{r, eval = FALSE}
